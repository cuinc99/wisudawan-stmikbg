...
"></span>
    <div class="wrap">
        <ul class="flexblock">
            <li>
                <img class="img aligncenter size-20" align="middle" src="{{ asset('/slide/logo_stmik.png') }}" alt="STMIKBG">
            </li>
        </ul>
        <h2 class="text-landing text-shadow"><strong>{{ $setting->stmik_judul }}</strong></h2>
        <h5 class="text-landing text-shadow"><strong>{!! $setting->stmik_deskripsi  !!}</strong></h5>
    </div>
</section>

@foreach($prodis as $prodi)
    @if($prodi->kampus == 'stmikbg' AND !$prodi->wisudawan->isEmpty())
    <section class="bg-gradient-r aligncenter">
    ...
    @foreach($prodi->wisudawan()->orderBy('angkatan_id', 'desc', 'nim')->get() as $wisudawan)
        @if($wisudawan->terbaik == false && $wisudawan->berprestasi == false)
            <section class="fullscreen bg-gradient-h" style="{{ $prodi->color }}">
                <div class="card-30">
                    <figure>
